...
class Approx_Q():
    def __init__(self,state_size, action_size, dinotype_num, seed):
        ...
        self.t_step = 0
        self.action_size = action_size
        #self.Q_table = np.full((8,8,4),0.0) #state_size = 8
        self.theta = np.full((4, 3),0.01)
        self.last_action = None
        self.last_observation = None
        self.statespace = []
        self.accumulated_reward = 0
    ...
    def step(self,state, action, reward, next_state, done, test, num_ep, map_update):
        self.t_step +=1
        last_state = state
        loc = next_state
        curr_state = np.array([loc[0],loc[1],self.get_prob(loc)])
        self.accumulated_reward += reward

        # update Q value
        pre_Q = copy.deepcopy(self.get_Q_value(curr_state, action))

        max_Q = []
        for i in range(self.action_size):
            max_Q.append(self.get_Q_value(curr_state,i))

        sa = np.array([curr_state[0], curr_state[1], curr_state[2], action])
        delta = reward + self.gamma * max(max_Q) - pre_Q

        new_theta = self.theta - self.alpha * delta * sa

        if not test:
            #self.Q_table[last_state[0]][last_state[1]][self.last_action] = new_Q
            self.theta = new_theta

            if not self.visitMap[loc[0]][loc[1]] and reward != 0 and map_update:
                self.visitMap[loc[0]][loc[1]] = 1
                #self.probMap[loc[0]][loc[1]]  = (self.probMap[loc[0]][loc[1]] * (num_ep-1) + 1)/num_ep
                self.probMap[loc[0]][loc[1]] = 1

    def get_Q_value(self, state, action):
        # print "battery level",battery
        #q_val = self.Q_table[state[0]][state[1]][action]
        sa = np.array([state[0], state[1], state[2], action])
        q_val = np.dot(self.theta, sa)
        return q_val

    def get_prob(self,loc):
        return self.probMap[loc[0]][loc[1]]
    ...
    def act(self, state, done):
        ...
        if random.random() > self.epsilon:
            Q_temp = []
            for i in range(self.action_size):
                Q_temp.append(self.get_Q_value(curr_state, i))
            temp = np.argmax(Q_temp)
        else:
            temp = random.randrange(self.action_size)
    ...
    def reset(self):
        ...
        self.action_size = 4
        # self.Q_table = np.full((8,8,4),0.0) #state_size = 8
        self.theta = np.array([0.1, 0.1, 0.1, 0.1])     # reset theta
        self.last_action = None
        self.last_observation = None
        self.statespace = []
        self.accumulated_reward = 0
...
class Fully_Agent():
    ...
    def step(self, state, action, reward, next_step, done,test, map_update):
        ...
        if self.t_step == 0:
            ...
            if len(self.memory) > BATCH_SIZE and not test:
                experience = self.memory.sample()
                self.learn(experience, GAMMA)

                # update proMap
                if not test:
                    if not self.visitMap[state[0]][state[1]] and reward != -1 and map_update:
